...
| ----------------------------------------------------------- | ------ | ----------- |
| `imessage`, `imessage-photon`, `whatsapp`, `sms`, `headless` | chat   | Terms render in the chat thread; the user approves by replying. No browser, no passkey (card-on-file purchases). |
| `chatgpt`, `claude` (the ChatGPT/Claude MCP Apps directories) | app    | Terms render inline in the app's own widget, same approval flow as chat — reply/tap "yes". ALSO has a browser available, so a passkey (agentic) card still goes through the browser wallet; no-passkey rails complete inline. |
| `claudecode`, `codex`, `browser`, `mobileapp`               | rich   | The user approves in the browser wallet (passkey, or no-passkey card-on-file). |
