...
- **Skills** — the deterministic Lane purchase flow and its setup sub-skills
  (`lane` router, `account-setup`, `wallet-setup`, `payment-execution`,
  `create-intent`, `agentic-checkout`). The agent runs everything; you only tap
  a passkey in the browser when asked.
- **MCP servers** (hosted) — `intent-mcp` (draft + approve a purchase intent)
  and `checkout-mcp` (autonomous cart + credential mint + payment + confirm),
  wired in automatically. No `claude mcp add` needed.
...
The MCP servers authenticate with your Lane API key. After `lane-cli init`
...
Defaults point at Lane **staging** (the only deployed environment today):

| Server       | Default URL                                          |
| ------------ | ---------------------------------------------------- |
| intent-mcp   | `https://mcp-staging.aws.getonlane.com/intent/mcp`   |
| checkout-mcp | `https://mcp-staging.aws.getonlane.com/checkout/mcp` |
