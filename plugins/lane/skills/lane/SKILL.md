...
1. **Draft + approve a purchase intent** (intent tools). Draft with `mcp__lane-mcp__intent_submit`; the user approves with a single passkey tap in the browser — this creates the VGS agentic intent server-side. The intent is persisted as `lint_*` with status `active`. A purchase intent may carry **N mandates** — one per distinct merchant.
2. **Drive the checkout autonomously** (checkout tools). After approval, call `mcp__lane-mcp__start_session({ intent_id })`. The engine resolves each mandate's merchant (its domain, else a SERP lookup of the name — no URL needed) and drives the full purchase in a real browser: login → find items → checkout → submit payment, looping the phases itself. You poll `get_session_status` and, whenever it needs the user (sign-in, shipping, OTP, passkey), surface the ask and poll `resume_session` until it's answered. See **Step 3** for the exact loop.
...
- **`draft_ready`** — a structured purchase plan the user should review, plus an `approval_url` and a `draft` object with full mandate details. Summarize it as a plain English purchase summary (items, merchants, total ceiling). **Save the mandate details now for reference:**
  ```text
...
The browser tab is just a viewport — `get_intent_status` is the source of truth, so you can always tell whether approval actually landed. Never depend on the user keeping a tab open. Two cases:
...
Once approval is confirmed (`kind: "complete"`), hand the intent to the checkout engine. It resolves each mandate's REAL merchant and drives the full purchase in a real browser (login, find items, checkout, submit payment). This is a real transaction against the real merchant in EVERY environment, staging included. There is no mock or sandbox merchant and no test code (e.g. there is no "123456 works"). You do not navigate, add to cart, or fill forms yourself; you orchestrate via three tools and relay human-in-the-loop asks.

**0. (Optional) Resolve the products first.** Before booting a browser you MAY call `mcp__lane-mcp__find_products({ mandate_id })` — a browser-free search that resolves each item's product URL from the merchant's OWN catalog search (falling back to a results page, then Google) and persists the URLs onto the mandate so the later `start_session` navigates straight to them. Use it especially when the user described items loosely (no link): it returns `products` (resolved) and `unresolved`, so on `outcome: "partial"`/`needs_human` you can surface the unresolved items and ask the user for a link BEFORE committing a session, instead of discovering the gap mid-checkout. Skipping it is fine — `start_session`'s find-items phase resolves in-session anyway — but pre-resolving gives the user a cleaner chance to clarify a vague item.

**1. Start.** Call `mcp__lane-mcp__start_session({ intent_id: "lint_..." })`. It returns immediately with `outcome: "running"` and boots a detached background run (login, find items, checkout, submit payment, per mandate). No merchant URL is needed.
...
**3. On `needs_human` (resume loop).** A suspend returns `outcome: "needs_human"` with a `suspend` payload (`prompt`, `ask_url`, `ask_id`, `kind`). These are REAL credentials/codes for the real merchant: the user's own password, or a one-time code the merchant actually sends to the user.
  - **TELL THE USER, in the terminal, the moment a `needs_human` ask fires.** Post a clear, standalone message: WHAT you need (sign in to the merchant / a one-time code / a shipping address / clarify which item) and WHERE to do it (their open Lane wallet — include the `ask_url`). Do NOT silently start polling, and do NOT bury the ask under a spinner or a "still working…" line. The user is often watching the terminal, not the wallet, and a dashboard popup alone will be missed — the run then looks "frozen" when it's actually waiting on them. Make the request impossible to miss BEFORE you poll.
