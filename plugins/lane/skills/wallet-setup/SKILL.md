...
When the user has at least one card and `lane-cli wallet enable-agentic` reports "Already enabled", the wallet is ready. Move to the purchase flow (draft via `lane-mcp` `intent_submit` → user approves with a passkey → `start_session` drives the checkout).
