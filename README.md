...
passkey, and Lane's `lane-mcp` autonomously completes the purchase at the
merchant. Bundles the Lane skills, the single `lane-mcp` server,
...
See [`codex/README.md`](./codex/README.md) — one block in `~/.codex/config.toml`
and an env var, with a copy-pasteable fragment in
[`codex/config.toml`](./codex/config.toml). Same hosted `lane-mcp`, same API key,
same account: a card you add from Claude Code works there with no extra setup.
...
**production**: `https://mcp.getonlane.com/mcp` (override with `LANE_MCP_HOST`).
There is nothing to configure at install — the API key is filled in
automatically after `lane-cli init` (see step 1 above).
...
- one **MCP server** (`lane-mcp`, declared in
  `plugins/lane/.claude-plugin/plugin.json`) that makes authenticated calls to
  Lane's hosted endpoint.
